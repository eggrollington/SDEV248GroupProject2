...
func _on_swordhitbox_area_entered(area: Area2D):
	print(">>> Sword hit area: ", area.name, " | Parent: ", area.get_parent().name)
	var enemy = area.get_parent()
	if enemy.is_in_group("enemies") and enemy.has_method("take_damage"):
		if area.name == "Hurtbox":print(">>> Enemy hit via hurtbox! Sending 1 damage")
		enemy.take_damage(1)
	else:
		print(">>> Ignored non-hurtbox area: ", area.name)
